...
export default function TeamVisualTree({ loading, fetchFocusWindow, variant = "user" }: Props) {
  ...
  const [viewDepth, setViewDepth] = useState(5);
  ...
  const fetcher = fetchFocusWindow ?? ((code?: string, depth?: number) => getMyTeamFocusWindow(code, depth));

  const loadWindow = useCallback(
    async (userCode?: string, depth = viewDepth) => {
      try {
        setFetching(true);
        setError(null);
        const res = await fetcher(userCode, depth);
  ...
  useEffect(() => {
    void loadWindow(focusUserCode, viewDepth);
  }, [viewDepth, focusUserCode, loadWindow]);

  const handleFocus = (userCode?: string) => {
    setFocusUserCode(userCode);
  };
  ...
  return (
    <div>
      ...
      <div className="rounded-3xl p-[1px] bg-gradient-to-br from-indigo-500/40 via-purple-500/30 to-transparent">
        <div className="bg-[#0b0f1a]/90 rounded-3xl border border-white/10 p-4 min-h-[50vh] max-h-[75vh] overflow-auto">
          {effectiveLoading && <div className="text-center py-10 text-slate-400 text-sm">Loading tree...</div>}
